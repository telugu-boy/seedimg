...
template<typename T>
using slut
    = std::array<std::array<float, seedimg::img::MAX_PIXEL_VALUE + 1>, sizeof(T) / sizeof(typename T::value_type)>;
} // namespace seedimg

namespace seedimg::filters {

static constexpr smat const SEPIA_MAT = {0.393f, 0.349f, 0.272f, 0.769f, 0.686f, 0.534f, 0.189f, 0.168f, 0.131f};
static constexpr smat const GRAYSCALE_LUM_MAT
    = {0.2126f, 0.2126f, 0.2126f, 0.7152f, 0.7152f, 0.7152f, 0.0722f, 0.0722f, 0.0722f};
...
void apply_mat_lut(simg& inp_img, simg& res_img, const seedimg::slut<seedimg::smat>& lut,
                   const lutvec& vec = {0, 0, 0});
void apply_mat_lut_i(simg& inp_img, const seedimg::slut<seedimg::smat>& lut, const lutvec& vec = {0, 0, 0});
...
inline smat generate_hue_mat(float angle) {
    ...
    return {0.213f + cosr * 0.787f - sinr * 0.213f, 0.213f - cosr * 0.213f + sinr * 0.143f,
            0.213f - cosr * 0.213f - sinr * 0.787f, 0.715f - cosr * 0.715f - sinr * 0.715f,
            0.715f + cosr * 0.285f + sinr * 0.140f, 0.715f - cosr * 0.715f + sinr * 0.715f,
            0.072f - cosr * 0.072f + sinr * 0.928f, 0.072f - cosr * 0.072f - sinr * 0.283f,
    ...
    for (std::size_t i = 1; i < mats.size(); i++) {
        for (std::size_t j = 0; j < 4; j++) {
            for (std::size_t k = 0; k < 4; k++) {
                res[j * 4 + k] = res[j * 4] * mats[i][0 + j] + res[j * 4 + 1] * mats[i][4 + j]
    ...
    for (std::size_t i = 1; i < mats.size(); i++) {
        for (std::size_t j = 0; j < 3; j++) {
            for (std::size_t k = 0; k < 3; k++) {
                res[j * 3 + k]
                    = res[j * 3] * mats[i][0 + j] + res[j * 3 + 1] * mats[i][3 + j] + res[j * 3 + 2] * mats[i][6 + j];
            }
        }
    }
    return res;
...
class filterchain {
  ...
  public:
    ...
    template<class F, class... Args> filterchain& add(F&& func, Args&&... args) {
        filters.push_back([&](simg& input, simg& output) -> void { func(input, output, std::forward<Args>(args)...); });
...
class filterchain_i {
  ...
  public:
     ...
     * @param args custom arguments to pass to callable.
     */
    template<class F, class... Args> filterchain_i& add(F&& func, Args&&... args) {
    ...
    filterchain_i& eval(anim& imgs, simg_int start = 0, simg_int end = 0) {
        if (end == 0 || end > imgs.size()) end = imgs.size();
        if (end <= start) // if nothing to do or invalid range.
            return *this;
