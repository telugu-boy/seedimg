...
void horizontal_blur_i_single_worker(
    simg& inp_img, simg& res_img, simg_int start, simg_int end, unsigned int blur_level) {
    for (simg_int y = start; y < end; ++y) {
        ...
        for (simg_int x = blur_level; x < inp_img->width() - blur_level; ++x) {
            ...
            r         = static_cast<simg_int>(std::max(static_cast<int_fast64_t>(0),
                                               static_cast<int_fast64_t>(r) + static_cast<int_fast64_t>(pix.r)
                                                   - static_cast<int_fast64_t>(prev.r)));
            g         = static_cast<simg_int>(std::max(static_cast<int_fast64_t>(0),
                                               static_cast<int_fast64_t>(g) + static_cast<int_fast64_t>(pix.g)
                                                   - static_cast<int_fast64_t>(prev.g)));
            b         = static_cast<simg_int>(std::max(static_cast<int_fast64_t>(0),
                                               static_cast<int_fast64_t>(b) + static_cast<int_fast64_t>(pix.b)
        ...
        for (simg_int x = inp_img->width() - blur_level; x < inp_img->width(); ++x) {
...
void vertical_blur_i_single_worker(
    simg& inp_img, simg& res_img, simg_int start, simg_int end, unsigned int blur_level) {
    for (simg_int x = start; x < end; ++x) {
        ...
        for (simg_int y = inp_img->height() - blur_level; y < inp_img->height(); ++y) {
...
void horizontal_blur_single_i(simg& inp_img, simg& res_img, unsigned int blur_level) {
    seedimg::utils::hrz_thread(horizontal_blur_i_single_worker, inp_img, res_img, blur_level);
}

void vertical_blur_single_i(simg& inp_img, simg& res_img, unsigned int blur_level) {
    seedimg::utils::vrt_thread(vertical_blur_i_single_worker, inp_img, res_img, blur_level);
}
...
unsigned int clamped_blur_level(unsigned int blur_level, simg_int width, simg_int height) {
    return static_cast<unsigned int>(std::min(
        static_cast<int>(blur_level), std::min(static_cast<int>((width - 1)) / 2, static_cast<int>((height - 1)) / 2)));
}

namespace seedimg::filters {
void h_blur_i(simg& inp_img, unsigned int blur_level, std::uint8_t it) {
    if (blur_level == 0) return;
    blur_level   = clamped_blur_level(blur_level, inp_img->width(), inp_img->height());
    ...
    if (it % 2 == 0 || it == 1) inp_img.reset(res_img.release());
}
...
void blur_i(simg& inp_img, unsigned int blur_level, std::uint8_t it) {
    ...
    blur_level   = clamped_blur_level(blur_level, inp_img->width(), inp_img->height());
    auto res_img = seedimg::make(inp_img->width(), inp_img->height());
    for (std::uint8_t i = 0; i < it; ++i) { box_blur_single(inp_img, res_img, blur_level); }
}
} // namespace seedimg::filters
