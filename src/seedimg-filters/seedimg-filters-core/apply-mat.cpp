...
void apply_mat_lut_worker(simg& inp_img, simg& res_img, simg_int start, simg_int end,
                          const seedimg::slut<seedimg::smat>& lut, const std::array<float, 3> vec) {
    for (; start < end; ++start) {
        for (simg_int x = 0; x < inp_img->width(); ++x) {
            seedimg::pixel pix         = inp_img->pixel(x, start);
            res_img->pixel(x, start).r = seedimg::utils::clamp(vec[0] + lut[0][pix.r] + lut[3][pix.g] + lut[6][pix.b],
                                                               0, seedimg::img::MAX_PIXEL_VALUE);
            res_img->pixel(x, start).g = seedimg::utils::clamp(vec[1] + lut[1][pix.r] + lut[4][pix.g] + lut[7][pix.b],
                                                               0, seedimg::img::MAX_PIXEL_VALUE);
            res_img->pixel(x, start).b = seedimg::utils::clamp(vec[2] + lut[2][pix.r] + lut[5][pix.g] + lut[8][pix.b],
                                                               0, seedimg::img::MAX_PIXEL_VALUE);
        }
    }
}
