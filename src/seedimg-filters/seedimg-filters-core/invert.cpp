...
void invert(simg& inp_img, simg& res_img) {
    ...
    for (std::size_t i = 0; i < workers.size(); i++) {
        workers.at(i) = std::thread(invert_worker, std::ref(inp_img), std::ref(res_img), start_end.at(i).first,
...
void invert_a(simg& inp_img, simg& res_img, bool invert_alpha_only) {
    ...
    if (invert_alpha_only) {
        for (std::size_t i = 0; i < workers.size(); i++) {
            workers.at(i) = std::thread(invert_worker_alpha_only, std::ref(inp_img), std::ref(res_img),
                                        start_end.at(i).first, start_end.at(i).second);
        }
    } else {
        for (std::size_t i = 0; i < workers.size(); i++) {
            workers.at(i) = std::thread(invert_worker_alpha, std::ref(inp_img), std::ref(res_img),
                                        start_end.at(i).first, start_end.at(i).second);
        }
    }
    for (std::size_t i = 0; i < workers.size(); ++i) workers.at(i).join();
