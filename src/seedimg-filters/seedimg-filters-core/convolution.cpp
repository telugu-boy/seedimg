...
void seedimg::filters::convolution(simg& input, std::vector<std::vector<float>> kernel) {
    if (kernel.size() == 0 || kernel[0].size() == 0) return;
    ...
    for (simg_int y = 0; y < kh; ++y)
        for (simg_int x = 0; x < kw; ++x)
            norm_kernel[kh - y - 1][kw - x - 1] = kernel[y][x] / (std::signbit(kernel[y][x]) ? neg_sum : pos_sum);
                    ...
                    auto pix = input->pixel(
                        static_cast<simg_int>(
                            static_cast<unsigned long long>(std::llabs(
                                static_cast<long long>(x) + static_cast<long long>(dx) - static_cast<long long>(ko_x)))
                            % input->width()),
                        static_cast<simg_int>(
                            static_cast<unsigned long long>(std::llabs(
                                static_cast<long long>(y) + static_cast<long long>(dy) - static_cast<long long>(ko_y)))
                            % input->height()));
