...
void seedimg::filters::convolution(simg& input, std::vector<std::vector<float>> kernel) {
    seedimg::point kernel_dims{kernel.size() ? kernel[0].size() : 0, kernel.size()};
    // NOTE: only square aka. NxN kernels are allowed as they're symmetric and
    //       thus very easy to implement, if not then just ignore.
    if (kernel_dims.x != kernel_dims.y || kernel_dims.x == 0) return;

    {
        // will be used as the divisor to normalize all the kernel elements.
        auto kernmatrix_normdiv
            = std::abs(std::accumulate(kernel.begin(), kernel.end(), 0.0, [](float s, auto r) -> float {
                  return s + std::accumulate(r.begin(), r.end(), 0.0);
              }));

        for (simg_int r = 0; r < kernel_dims.y; ++r)
            std::transform(kernel[r].begin(), kernel[r].end(), kernel[r].begin(), [&](auto elem) -> float {
                return elem / kernmatrix_normdiv;
            });
    }
        ...
        for (simg_int x = 0; x < input->width(); ++x) {
            ...
            for (std::size_t dy = 0; dy < kernel_dims.y; ++dy) {
                for (std::size_t dx = 0; dx < kernel_dims.x; ++dx) {
                    // NOTE: for edge handling a mixture of mirroring and
                    // wrapping is done. wrapping when it reaches the end,
                    // mirroring when it reaches the start.
                    auto pix = input->pixel(
                        static_cast<simg_int>(static_cast<unsigned long long>(
                                                  std::llabs(static_cast<long long>(x) + static_cast<long long>(dx)
                                                             - static_cast<long long>(kernel_origin.x)))
                                              % input->width()),
                        static_cast<simg_int>(static_cast<unsigned long long>(
                                                  std::llabs(static_cast<long long>(y) + static_cast<long long>(dy)
