...
static constexpr seedimg::smat const rgb_ycbcr_bt601_mat
    = {298.082f / 256.0f, 298.082f / 256.0f, 298.082f / 256.0f,  0.0f / 256.0f, -100.291f / 256.0f,
       516.412f / 256.0f, 408.583f / 256.0f, -208.120f / 256.0f, 0.0f / 256.0f};

static constexpr seedimg::lutvec const rgb_ycbcr_bt601_vec = {-222.921f, 135.576f, -276.836f};

static constexpr seedimg::slut<seedimg::smat> const rgb_ycbcr_bt601_lut = seedimg::utils::gen_lut(rgb_ycbcr_bt601_mat);

void hsv2rgb_worker(simg& inp_img, simg& res_img, simg_int start, simg_int end) {
    for (; start < end; start++) {
        for (simg_int x = 0; x < inp_img->width(); ++x) {
            ...
            struct {
                ...
                float s;
                float v;
            } pix{static_cast<float>(pix_src.h) * 2, static_cast<float>(pix_src.s) / 100,
            ...
            res_img->pixel(x, start) = {{static_cast<std::uint8_t>(componentsp[0] * 255)},
...
void ycbcr_jpeg2rgb_worker(simg& inp_img, simg& res_img, simg_int start, simg_int end) {
    for (; start < end; ++start) {
        for (simg_int x = 0; x < inp_img->width(); ++x) {
            ...
            res_img->pixel(x, start).r = static_cast<std::uint8_t>(pix.y + 0 + jpeg_bcr1[pix.cr]);
            res_img->pixel(x, start).g = static_cast<std::uint8_t>(pix.y + jpeg_gcb2[pix.cb] + jpeg_bcr2[pix.cr]);
            res_img->pixel(x, start).b = static_cast<std::uint8_t>(pix.y + jpeg_gcb3[pix.cb] + 0);
        }
    }
}
...
void rgb(simg& inp_img, simg& res_img) {
    if (inp_img->colourspace() == seedimg::colourspaces::rgb) {
        ...
        seedimg::filters::apply_mat_lut(inp_img, res_img, rgb_ycbcr_bt601_lut, rgb_ycbcr_bt601_vec);
    }
