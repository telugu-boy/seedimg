...
void get_hue_kernel(int angle, cl_float16 *hue_kernel) {
  const float sinr = static_cast<float>(std::sin(angle * PI / 180));
  const float cosr = static_cast<float>(std::cos(angle * PI / 180));
  hue_kernel->s[0] = 0.213f + cosr * 0.787f - sinr * 0.213f;
  hue_kernel->s[1] = 0.715f - cosr * 0.715f - sinr * 0.715f;
  hue_kernel->s[2] = 0.072f - cosr * 0.072f + sinr * 0.928f;
  hue_kernel->s[3] = 0.213f - cosr * 0.213f + sinr * 0.143f;
  hue_kernel->s[4] = 0.715f + cosr * 0.285f + sinr * 0.140f;
  hue_kernel->s[5] = 0.072f - cosr * 0.072f - sinr * 0.283f;
  hue_kernel->s[6] = 0.213f - cosr * 0.213f - sinr * 0.787f;
  hue_kernel->s[7] = 0.715f - cosr * 0.715f + sinr * 0.715f;
  hue_kernel->s[8] = 0.072f + cosr * 0.928f + sinr * 0.072f;
}
...
void rotate_hue(simg &inp_img, simg &res_img, int angle) {
  ...
  // generate kernel that will be applied for each pixel
  cl_float16 hue_kernel_vec;
  get_hue_kernel(angle, &hue_kernel_vec);
  ...
  rotate_hue_ocl.setArg(0, hue_kernel_vec);
