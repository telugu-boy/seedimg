...
bool to(const std::string& filename, const simg& inp_img, float quality) {
    ...
    // '''''-'''''
    std::size_t success
        = WebPEncodeRGBA(reinterpret_cast<std::uint8_t*>(inp_img->data()),
                         static_cast<int>(inp_img->width()),
                         static_cast<int>(inp_img->height()),
                         static_cast<int>(inp_img->width() * static_cast<simg_int>(sizeof(seedimg::pixel))),
                         quality,
                         &output);
...
simg from(const std::string& filename) {
    ...
    return std::make_unique<seedimg::img>(
        static_cast<simg_int>(width), static_cast<simg_int>(height), reinterpret_cast<seedimg::pixel*>(decoded));
}
} // namespace webp
} // namespace seedimg::modules
