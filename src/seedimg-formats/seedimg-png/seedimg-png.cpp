...
simg from(const std::string& filename) {
    ...
    res_img          = seedimg::make(png_get_image_width(png_ptr, info_ptr), png_get_image_height(png_ptr, info_ptr));
    ...
    for (int pass = 0; pass < interlace_passes; pass++) {
        for (size_t y = 0; y < res_img->height(); y++)
            png_read_row(png_ptr, reinterpret_cast<png_bytep>(res_img->row(y)), nullptr);
    }

finalise:
    if (fp != nullptr) fclose(fp);
    if (info_ptr != nullptr) png_destroy_info_struct(png_ptr, &info_ptr);
    if (png_ptr != nullptr) png_destroy_read_struct(&png_ptr, nullptr, nullptr);
...
bool to(const std::string& filename, const simg& inp_img) {
    ...
    png_set_IHDR(png_ptr, info_ptr, static_cast<png_uint_32>(inp_img->width()),
                 static_cast<png_uint_32>(inp_img->height()), 8, PNG_COLOR_TYPE_RGBA, PNG_INTERLACE_NONE,
