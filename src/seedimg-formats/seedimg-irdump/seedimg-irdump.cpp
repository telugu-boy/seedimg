...
bool to(const std::string& filename, const simg& input) {
    ...
    } catch (std::fstream::failure) { return false; }

    const auto rowstride = input->width() * sizeof(pixel);
    for (simg_int r = 0; r < input->height(); ++r) {
        try {
            output.write(reinterpret_cast<char*>(input->row(r)), static_cast<std::streamsize>(rowstride));
        } catch (std::fstream::failure) { return false; }
    }
...
simg from(const std::string& filename) {
    ...
    auto image = seedimg::make(from_u32be(reinterpret_cast<std::uint8_t*>(rawinfo.width)),
    ...
    for (simg_int r = 0; r < image->height(); ++r) {
        try {
            input.read(reinterpret_cast<char*>(image->row(r)), static_cast<std::streamsize>(rowstride));
        } catch (std::fstream::failure) { return nullptr; }
    }
