...
simg seedimg::modules::jpeg::from(const std::string& filename) {
    ...
    jpeg_CreateDecompress(&jdec, JPEG_LIB_VERSION, static_cast<size_t>(sizeof(struct jpeg_decompress_struct)));
    ...
    for (simg_int y = 0; y < res_img->height(); ++y) {
        if (jpeg_read_scanlines(&jdec, &rowbuffer, 1) != 1) return nullptr;

        for (simg_int x = 0; x < res_img->width(); ++x) {
            res_img->pixel(x, y) = {{rowbuffer[3 * x]}, {rowbuffer[3 * x + 1]}, {rowbuffer[3 * x + 2]}, 0xFF};
        }
    }

finalise:
    if (errcode != -1) jpeg_finish_decompress(&jdec);
    jpeg_destroy_decompress(&jdec);
    std::fclose(input);
    if (rowbuffer != nullptr) delete[] rowbuffer;
    if (errcode == 0)
        return res_img;
    else
        return nullptr;
...
bool seedimg::modules::jpeg::to(const std::string& filename, const simg& image, uint8_t quality, bool progressive) {
    ...
    jpeg_CreateCompress(&jenc, JPEG_LIB_VERSION, static_cast<size_t>(sizeof(struct jpeg_compress_struct)));
    ...
    rowbuffer = new JSAMPLE[jenc.image_width * static_cast<simg_int>(jenc.input_components)];
    ...
    for (simg_int y = 0; y < image->height(); ++y) {
        ...
        if (jpeg_write_scanlines(&jenc, &rowbuffer, 1) != 1) return false;
    }
