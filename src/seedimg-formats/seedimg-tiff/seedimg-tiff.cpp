...
bool to(const std::string& filename, const anim& inp_anim) {
    ...
    if (img) {
        for (std::size_t i = 0; i < inp_anim.size(); ++i) {
            if (inp_anim[i]->width() > UINT32_MAX || inp_anim[i]->height() > UINT32_MAX) return false;
            ...
            unsigned char* buf = static_cast<unsigned char*>(
                _TIFFmalloc(static_cast<tmsize_t>(inp_anim[i]->width() * sizeof(seedimg::pixel))));

            for (simg_int y = 0; y < inp_anim[i]->height(); ++y) {
                std::copy(inp_anim[i]->row(y), inp_anim[i]->row(y + 1), reinterpret_cast<seedimg::pixel*>(buf));
                if (TIFFWriteScanline(img, buf, static_cast<uint32>(y), 0) < 0) { return false; }
            }
...
bool to(const std::string& filename, const simg& inp_img) {
    ...
    unsigned char* buf
        = static_cast<unsigned char*>(_TIFFmalloc(static_cast<tmsize_t>(inp_img->width() * sizeof(seedimg::pixel))));

    for (simg_int y = 0; y < inp_img->height(); ++y) {
        std::copy(inp_img->row(y), inp_img->row(y + 1), reinterpret_cast<seedimg::pixel*>(buf));
        if (TIFFWriteScanline(img, buf, static_cast<uint32>(y), 0) < 0) { return false; }
    }
    _TIFFfree(buf);
    return true;
...
anim from(const std::string& filename, std::size_t max_frames) {
    ...
    if (img) {
        do {
            ...
            TIFFReadRGBAImage(img, w, h, reinterpret_cast<uint32*>(decompressed->data()), 0);
            ...
            for (simg_int y = 0; y < h / 2; ++y) {
                ...
                std::copy(decompressed->row(h - y - 1), decompressed->row(h - y), decompressed->row(y));
