...
class img::img_impl {
  public:
    ...
    img_impl(simg_int w, simg_int h, colourspaces space = colourspaces::rgb)
        ...
        , width_{w}
        , height_{h} {
        data_ = static_cast<seedimg::pixel*>(
            std::malloc(static_cast<std::size_t>(height_ * width_) * sizeof(seedimg::pixel*)));
        if (data_ == nullptr) throw std::bad_alloc();
    }

    img_impl(simg_int w, simg_int h, seedimg::pixel* u_data, colourspaces space = colourspaces::rgb)
        : colourspace_{space}
        , width_{w}
        , height_{h}
        , data_{u_data} {}
    ...
    img_impl(img_impl const& img_)
        ...
        std::copy(img_.data(), img_.data() + img_.width_ * img_.height_, this->data());
    }
    ...
    seedimg::pixel& pixel(simg_int x, simg_int y) const noexcept { return data()[y * width() + x]; }

    seedimg::pixel& pixel(seedimg::point p) const noexcept { return pixel(p.x, p.y); }

    seedimg::pixel& pixel(simg_int x) const noexcept { return pixel(x / width(), x % width()); }

    seedimg::pixel& pixel_s(simg_int x, simg_int y) const {
        if (x >= width() || y >= height()) throw std::out_of_range("Coordinates out of range");
        return data()[y * width() + x];
    }
    seedimg::pixel& pixel_s(seedimg::point p) const { return pixel(p.x, p.y); }
    seedimg::pixel& pixel_s(simg_int x) const { return pixel(x / width(), x % width()); }

    seedimg::pixel* row(simg_int y) const noexcept { return data() + y * width(); }

    seedimg::pixel* row_s(simg_int y) const {
        if (y >= height()) throw std::out_of_range("Row out of range");
        return data() + y * width();
    }
    ...
    std::vector<std::pair<simg_int, simg_int>> start_end_rows() const noexcept {
        ...
        auto processor_count = std::min(this->height(), static_cast<simg_int>(std::thread::hardware_concurrency()));
        ...
        for (simg_int i = 0; i < processor_count * rows_per_thread; i += rows_per_thread)
    ...
    std::vector<std::pair<simg_int, simg_int>> start_end_cols() const noexcept {
        std::vector<std::pair<simg_int, simg_int>> res;
        auto processor_count = std::min(this->width(), static_cast<simg_int>(std::thread::hardware_concurrency()));
        if (processor_count == 0) processor_count = 1;
        res.reserve(static_cast<std::size_t>(processor_count));
        simg_int cols_per_thread = this->width() / processor_count;
        for (simg_int i = 0; i < processor_count * cols_per_thread; i += cols_per_thread)
    ...
    void         set_colourspace(seedimg::colourspaces colourspace) noexcept { this->colourspace_ = colourspace; }
...
img::img(simg_int w, simg_int h)
    : impl{new img_impl{w, h}} {}

img::img(simg_int w, simg_int h, seedimg::pixel* u_data)
    : impl{new img_impl{w, h, u_data}} {}

img::img(seedimg::img const& img_)
    : impl{new img_impl{*img_.impl}} {}

img::img(seedimg::img&& other) noexcept
...
std::vector<std::pair<simg_int, simg_int>> img::start_end_rows() const noexcept { return impl->start_end_rows(); }

std::vector<std::pair<simg_int, simg_int>> img::start_end_cols() const noexcept { return impl->start_end_cols(); }

seedimg::pixel& img::pixel(simg_int x, simg_int y) const noexcept { return impl->pixel(x, y); }

seedimg::pixel& img::pixel(seedimg::point p) const noexcept { return impl->pixel(p); }

seedimg::pixel& img::pixel(simg_int x) const noexcept { return impl->pixel(x); }

seedimg::pixel& img::pixel_s(simg_int x, simg_int y) const { return impl->pixel_s(x, y); }

seedimg::pixel& img::pixel_s(seedimg::point p) const { return impl->pixel(p.x, p.y); }
...
void uimg::set_colourspace(seedimg::colourspaces colourspace) noexcept { impl->set_colourspace(colourspace); }
} // namespace seedimg
